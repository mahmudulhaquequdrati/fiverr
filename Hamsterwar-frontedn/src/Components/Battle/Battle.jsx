...
const Battle = (props) => {
  ...
  useEffect(() => {
    const getHamsters = async () => {
      ...
      do {
        for (let i = 0; i <= 1; i++) {
          const response = await fetch("http://localhost:1345/hamsters/random");
  ...
  const vote = async (id) => {
    ...
    try {
      await Promise.all([
        fetch(
          `http://localhost:1345/hamsters/${winnerHamsterId}`,
          winnerRequest
        ),
        fetch(
          `http://localhost:1345/hamsters/${loosingHamsterId}`,
          looserRequest
        ),
      ]);
      setVoted(true);
